...
class WSStreamActor(out: ActorRef) extends Actor {
  ...
  def receive = {
    case msg: JsValue ⇒
      msg.validate[QuestionThreadIdChanged].foreach {
        changed ⇒ questionThreadId = Some(changed.questionThreadId)
      }
...
object WSStreamActor {
  ...
  case class QuestionThreadDataUpdated(questionThreadId: UUID, jsValue: JsValue)
